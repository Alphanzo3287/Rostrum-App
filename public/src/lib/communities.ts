...
export const deleteCommunity = (id: string) => supabase.rpc('delete_community', { p_community: id }).then(r => { if (r.error) throw r.error; });

export interface CommunityDebate {
  id: string; motion: string; format: string; status: string;
  scheduled_at: string | null; created_at: string; viewer_count: number | null;
}
/** Debates hosted in a community, newest/soonest first. */
export async function communityDebates(communityId: string): Promise<CommunityDebate[]> {
  const { data, error } = await supabase.from('debates')
    .select('id, motion, format, status, scheduled_at, created_at, viewer_count')
    .eq('community_id', communityId)
    .order('created_at', { ascending: false }).limit(50);
  if (error) throw error;
  return (data ?? []) as CommunityDebate[];
}
