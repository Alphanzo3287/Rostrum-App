...
export function CreateDebateScreen({ onCancel, onCreated }: {
  ...
  async function create() {
    ...
    try {
      ...
      if (deck.length) await uploadDeck(debate.id, deck);
      onCreated(debate.id);
    } catch (e: any) {
      ...
      <div style={{ maxWidth:860, margin:'0 auto', padding:'24px 24px 110px' }}>
        ...
        <div style={{ background:C.panel, border:`1px solid ${C.hair}`, borderRadius:10, padding:'22px 24px', minHeight:340 }}>
          ...
          {step === 3 && <>
            ...
            <div style={{ marginTop:18 }}>
              <Label>Slides <span style={{ color:C.faint, fontWeight:400 }}>(optional — image pages)</span></Label>
              <input type="file" accept="image/*" multiple style={{ marginTop:8, color:C.dim, fontFamily:ui, fontSize:12 }}
                onChange={e => setDeck(Array.from(e.target.files ?? []))} />
              {deck.length > 0 && <span style={{ fontFamily:ui, fontSize:12, color:C.jadeHi, marginLeft:8 }}>{deck.length} page(s)</span>}
            </div>
          </>}
        </div>
