...
import type { Side, Question } from '../lib/types';
import { C, ui, display, mono, solidGold, field } from '../lib/theme';
import { supabase } from '../lib/supabaseClient';

type Role = 'host' | 'moderator' | 'debater' | 'judge' | 'audience';

export function ContextRail({ debateId, role, tab, setTab }: {
  debateId: string; role: Role; tab: string; setTab: (t: string) => void;
}) {
  const tabs = role === 'host'  ? [['ros','Run'],['qa','Q&A'],['poll','Poll']]
