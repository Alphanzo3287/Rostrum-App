...
import {
  ...
  postToCommunity, deleteCommunityPost, deleteCommunity, communityDebates,
  type Community, type CommunityPost, type CommunityMember, type CommunityDebate,
...
export function CommunityScreen() {
  ...
  const canManage = !!community && !!user && (community.created_by === user.id || !!(profile as any)?.is_admin);

  const refresh = () => {
    if (!id) return;
    Promise.all([getCommunity(id), isMember(id), communityMembers(id), communityFeed(id), communityDebates(id)])
      .then(([c, m, mem, f, d]) => { setCommunity(c); setMember(m); setMembers(mem); setFeed(f); setDebates(d); })
  ...
  async function removeCommunity() {
    ...
    if (!confirm(`Delete "${community.name}"? This removes all its posts and members. This cannot be undone.`)) return;
    try { await deleteCommunity(id); nav('/communities'); }
    catch (e: any) { setErr(e?.message ?? 'Could not delete community'); }
  }
        ...
        {canManage && (
          <div style={{ marginTop: 10 }}>
            <button onClick={removeCommunity}
              style={{ background: 'transparent', border: `1px solid ${a(C.garnet, '55')}`, color: C.garnetHi,
                borderRadius: 8, padding: '6px 12px', fontFamily: ui, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
              Delete community
            </button>
          </div>
        )}
        ...
        <div>
          {debates.length > 0 && (
            <div style={{ marginBottom: 22 }}>
              <div style={{ fontFamily: ui, fontSize: 12, fontWeight: 800, letterSpacing: '.06em', textTransform: 'uppercase', color: C.faint, marginBottom: 10 }}>Debates</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {debates.map(d => {
                  const live = d.status === 'live' || d.status === 'assembly';
                  const upcoming = d.status === 'scheduled';
                  return (
                    <button key={d.id} onClick={() => nav(`/debate/${d.id}`)}
                      style={{ display: 'flex', alignItems: 'center', gap: 11, textAlign: 'left', cursor: 'pointer',
                        background: C.panel, border: `1px solid ${C.hair}`, borderRadius: 12, padding: '11px 14px' }}>
                      <span style={{ fontFamily: ui, fontSize: 9.5, fontWeight: 800, letterSpacing: '.05em', textTransform: 'uppercase',
                        padding: '3px 8px', borderRadius: 999, whiteSpace: 'nowrap',
                        background: live ? a(C.garnet, '1A') : upcoming ? a(C.cyan, '1A') : a(C.hair, '80'),
                        color: live ? C.garnetHi : upcoming ? C.cyan : C.faint }}>
                        {live ? '● Live' : upcoming ? 'Upcoming' : 'Ended'}
                      </span>
                      <span style={{ flex: 1, minWidth: 0, fontFamily: ui, fontSize: 13.5, fontWeight: 600, color: C.ink,
                        overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{d.motion}</span>
                    </button>
                  );
                })}
              </div>
