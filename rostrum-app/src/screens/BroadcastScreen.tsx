              ...
              {speaker && (
                <div style={{ position:'absolute', bottom:14, right:14, width:240, height:135,
                  borderRadius:10, overflow:'hidden', border:`2px solid ${
                    speaker.side === 'prop' ? C.jadeHi : speaker.side === 'opp' ? C.garnetHi : C.gold}`,
                  boxShadow:'0 8px 24px rgba(0,0,0,0.5)' }}>
                  <VideoTile member={speaker} active size="tile" />
                </div>
              )}
            </>
          ) : (
            speaker
              ? <VideoTile member={speaker} active size="stage" />
...
function SideCard({ label, tone, members, active }: {
  label: string; tone: string; members: { identity:string; name:string }[] | any[]; active: boolean;
}) {
  return (
