...
import {
  ...
  withdrawFromTournament, deleteTournament, type Tournament, type TournamentEntrant,
...
export function TournamentScreen() {
  ...
  const refresh = () => {
    ...
    Promise.all([getTournament(id), tournamentEntrants(id), isRegistered(id)])
      .then(([tt, e, r]) => { setT(tt); setEntrants(e); setRegistered(r); })
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '26px 20px 70px' }}>
      ...
      {/* bracket placeholder (Phase 2) */}
      <div style={{ marginTop: 26, padding: '22px', borderRadius: 14, background: a(C.gold, '08'), border: `1px dashed ${a(C.gold, '3A')}`, textAlign: 'center' }}>
        <div style={{ fontFamily: ui, fontSize: 13, color: C.dim, lineHeight: 1.5 }}>
          🏆 Once registration fills, the bracket will be seeded and each match played as a live debate. Bracket view is coming next.
        </div>
      </div>
